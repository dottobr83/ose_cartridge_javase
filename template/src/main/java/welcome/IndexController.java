package welcome;

import org.springframework.boot.SpringApplication;
import org.springframework.boot.autoconfigure.EnableAutoConfiguration;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.ResponseBody;

@Controller
@EnableAutoConfiguration
public class IndexController {

    @RequestMapping("/")
    @ResponseBody
    String home() {
    	StringBuffer buffer = new StringBuffer();
    	buffer.append("<!doctype html>");
    	buffer.append("<html lang=\"en\">");
    	buffer.append("<head>");
    	buffer.append("  <meta charset=\"utf-8\">");
    	buffer.append("  <meta http-equiv=\"X-UA-Compatible\" content=\"IE=edge,chrome=1\">");
    	buffer.append("  <title>Welcome to OpenShift</title>");
    	buffer.append("  ");
    	buffer.append("<style>");
    	buffer.append("");
    	buffer.append("/*!");
    	buffer.append(" * Bootstrap v3.0.0");
    	buffer.append(" *");
    	buffer.append(" * Copyright 2013 Twitter, Inc");
    	buffer.append(" * Licensed under the Apache License v2.0");
    	buffer.append(" * http://www.apache.org/licenses/LICENSE-2.0");
    	buffer.append(" *");
    	buffer.append(" * Designed and built with all the love in the world @twitter by @mdo and @fat.");
    	buffer.append(" */");
    	buffer.append("");
    	buffer.append("  .logo {");
    	buffer.append("    background-size: cover;");
    	buffer.append("    height: 58px;");
    	buffer.append("    width: 180px;");
    	buffer.append("    margin-top: 6px;");
    	buffer.append("    background-image: url(data:image/svg+xml;base64,);");
    	buffer.append("  }");
    	buffer.append(".logo a {");
    	buffer.append("  display: block;");
    	buffer.append("  width: 100%;");
    	buffer.append("  height: 100%;");
    	buffer.append("}");
    	buffer.append("*, *:before, *:after {");
    	buffer.append("    -moz-box-sizing: border-box;");
    	buffer.append("    box-sizing: border-box;");
    	buffer.append("}");
    	buffer.append("aside,");
    	buffer.append("footer,");
    	buffer.append("header,");
    	buffer.append("hgroup,");
    	buffer.append("section{");
    	buffer.append("  display: block;");
    	buffer.append("}");
    	buffer.append("body {");
    	buffer.append("  color: #404040;");
    	buffer.append("  font-family: \"Helvetica Neue\",Helvetica,\"Liberation Sans\",Arial,sans-serif;");
    	buffer.append("  font-size: 14px;");
    	buffer.append("  line-height: 1.4;");
    	buffer.append("}");
    	buffer.append("");
    	buffer.append("html {");
    	buffer.append("  font-family: sans-serif;");
    	buffer.append("  -ms-text-size-adjust: 100%;");
    	buffer.append("  -webkit-text-size-adjust: 100%;");
    	buffer.append("}");
    	buffer.append("ul {");
    	buffer.append("    margin-top: 0;");
    	buffer.append("}");
    	buffer.append(".container {");
    	buffer.append("  margin-right: auto;");
    	buffer.append("  margin-left: auto;");
    	buffer.append("  padding-left: 15px;");
    	buffer.append("  padding-right: 15px;");
    	buffer.append("}");
    	buffer.append(".container:before,");
    	buffer.append(".container:after {");
    	buffer.append("  content: \" \";");
    	buffer.append("  /* 1 */");
    	buffer.append("");
    	buffer.append("  display: table;");
    	buffer.append("  /* 2 */");
    	buffer.append("");
    	buffer.append("}");
    	buffer.append(".container:after {");
    	buffer.append("  clear: both;");
    	buffer.append("}");
    	buffer.append(".row {");
    	buffer.append("  margin-left: -15px;");
    	buffer.append("  margin-right: -15px;");
    	buffer.append("}");
    	buffer.append(".row:before,");
    	buffer.append(".row:after {");
    	buffer.append("  content: \" \";");
    	buffer.append("  /* 1 */");
    	buffer.append("");
    	buffer.append("  display: table;");
    	buffer.append("  /* 2 */");
    	buffer.append("");
    	buffer.append("}");
    	buffer.append(".row:after {");
    	buffer.append("  clear: both;");
    	buffer.append("}");
    	buffer.append(".col-sm-6, .col-md-6, .col-xs-12 {");
    	buffer.append("  position: relative;");
    	buffer.append("  min-height: 1px;");
    	buffer.append("  padding-left: 15px;");
    	buffer.append("  padding-right: 15px;");
    	buffer.append("}");
    	buffer.append(".col-xs-12 {");
    	buffer.append("  width: 100%;");
    	buffer.append("}");
    	buffer.append("");
    	buffer.append("@media (min-width: 768px) {");
    	buffer.append("  .container {");
    	buffer.append("    width: 750px;");
    	buffer.append("  }");
    	buffer.append("  .col-sm-6 {");
    	buffer.append("    float: left;");
    	buffer.append("  }");
    	buffer.append("  .col-sm-6 {");
    	buffer.append("    width: 50%;");
    	buffer.append("  }");
    	buffer.append("}");
    	buffer.append("");
    	buffer.append("@media (min-width: 992px) {");
    	buffer.append("  .container {");
    	buffer.append("    width: 970px;");
    	buffer.append("  }");
    	buffer.append("  .col-md-6 {");
    	buffer.append("    float: left;");
    	buffer.append("  }");
    	buffer.append("  .col-md-6 {");
    	buffer.append("    width: 50%;");
    	buffer.append("  }");
    	buffer.append("}");
    	buffer.append("@media (min-width: 1200px) {");
    	buffer.append("  .container {");
    	buffer.append("    width: 1170px;");
    	buffer.append("  }");
    	buffer.append("}");
    	buffer.append("");
    	buffer.append("a {");
    	buffer.append("  color: #069;");
    	buffer.append("  text-decoration: none;");
    	buffer.append("}");
    	buffer.append("a:hover {");
    	buffer.append("  color: #EA0011;");
    	buffer.append("  text-decoration: underline;");
    	buffer.append("}");
    	buffer.append("hgroup {");
    	buffer.append("  margin-top: 50px;");
    	buffer.append("}");
    	buffer.append("footer {");
    	buffer.append("    margin: 50px 0 25px;");
    	buffer.append("}");
    	buffer.append("h1, h2, h3 {");
    	buffer.append("  color: #000;");
    	buffer.append("  line-height: 1.38em;");
    	buffer.append("  margin: 1.5em 0 .3em;");
    	buffer.append("}");
    	buffer.append("h1 {");
    	buffer.append("  font-size: 25px;");
    	buffer.append("  font-weight: 300;");
    	buffer.append("  border-bottom: 1px solid #fff;");
    	buffer.append("  margin-bottom: .5em;");
    	buffer.append("}");
    	buffer.append("h1:after {");
    	buffer.append("  content: \"\";");
    	buffer.append("  display: block;");
    	buffer.append("  width: 100%;");
    	buffer.append("  height: 1px;");
    	buffer.append("  background-color: #ddd;");
    	buffer.append("}");
    	buffer.append("h2 {");
    	buffer.append("  font-size: 19px;");
    	buffer.append("  font-weight: 400;");
    	buffer.append("}");
    	buffer.append("h3 {");
    	buffer.append("  font-size: 15px;");
    	buffer.append("  font-weight: 400;");
    	buffer.append("  margin: 0 0 .3em;");
    	buffer.append("}");
    	buffer.append("p {");
    	buffer.append("  margin: 0 0 2em;");
    	buffer.append("}");
    	buffer.append("p + h2 {");
    	buffer.append("  margin-top: 2em;");
    	buffer.append("}");
    	buffer.append("html {");
    	buffer.append("  background: #f5f5f5;");
    	buffer.append("  height: 100%;");
    	buffer.append("}");
    	buffer.append("code {");
    	buffer.append("  background-color: white;");
    	buffer.append("  border: 1px solid #ccc;");
    	buffer.append("  padding: 1px 5px;");
    	buffer.append("  color: #888;");
    	buffer.append("}");
    	buffer.append("pre {");
    	buffer.append("  display: block;");
    	buffer.append("  padding: 13.333px 20px;");
    	buffer.append("  margin: 0 0 20px;");
    	buffer.append("  font-size: 13px;");
    	buffer.append("  line-height: 1.4;");
    	buffer.append("  background-color: #fff;");
    	buffer.append("  border-left: 2px solid rgba(120,120,120,0.35);");
    	buffer.append("  white-space: pre;");
    	buffer.append("  white-space: pre-wrap;");
    	buffer.append("  word-break: normal;");
    	buffer.append("  word-wrap: break-word;");
    	buffer.append("  overflow: auto;");
    	buffer.append("  font-family: Menlo,Monaco,\"Liberation Mono\",Consolas,monospace !important;");
    	buffer.append("}");
    	buffer.append("");
    	buffer.append("</style>");
    	buffer.append("");
    	buffer.append("</head>");
    	buffer.append("<body>");
    	buffer.append("");
    	buffer.append("<section class='container'>");
    	buffer.append("          <hgroup>");
    	buffer.append("            <h1>Welcome to your Java SE application on OpenShift</h1>");
    	buffer.append("          </hgroup>");
    	buffer.append("");
    	buffer.append("");
    	buffer.append("        <div class=\"row\">");
    	buffer.append("          <section class='col-xs-12 col-sm-6 col-md-6'>");
    	buffer.append("            <section>");
    	buffer.append("              <h2>Deploying code changes</h2>");
    	buffer.append("                <p>OpenShift uses the <a href=\"http://git-scm.com/\">Git version control system</a> for your source code, and grants you access to it via the Secure Shell (SSH) protocol. In order to upload and download code to your application you need to give us your <a href=\"https://www.openshift.com/developers/remote-access\">public SSH key</a>. You can upload it within the web console or install the <a href=\"https://www.openshift.com/developers/rhc-client-tools-install\">RHC command line tool</a> and run <code>rhc setup</code> to generate and upload your key automatically.</p>");
    	buffer.append("");
    	buffer.append("                <h3>Working in your local Git repository</h3>");
    	buffer.append("                <p>If you created your application from the command line and uploaded your SSH key, rhc will automatically download a copy of that source code repository (Git calls this 'cloning') to your local system.</p>");
    	buffer.append("");
    	buffer.append("                <p>If you created the application from the web console, you'll need to manually clone the repository to your local system. Copy the application's source code Git URL and then run:</p>");
    	buffer.append("");
    	buffer.append("<pre>$ git clone &lt;git_url&gt; &lt;directory_to_create&gt;");
    	buffer.append("");
    	buffer.append("# Within your project directory");
    	buffer.append("# Commit your changes and push to OpenShift");
    	buffer.append("");
    	buffer.append("$ git commit -a -m 'Some commit message'");
    	buffer.append("$ git push</pre>");
    	buffer.append("");
    	buffer.append("");
    	buffer.append("                  <ul>");
    	buffer.append("                    <li><a href=\"https://www.openshift.com/developers/deploying-and-building-applications\">Learn more about deploying and building your application</a></li>");
    	buffer.append("                    <li>See the README file in your local application Git repository for more information on the options for deploying applications.</li>");
    	buffer.append("                  </ul>");
    	buffer.append("");
    	buffer.append("            </section>");
    	buffer.append("");
    	buffer.append("          </section>");
    	buffer.append("          <section class=\"col-xs-12 col-sm-6 col-md-6\">");
    	buffer.append("");
    	buffer.append("                <h2>Managing your application</h2>");
    	buffer.append("");
    	buffer.append("                <h3>Web Console</h3>");
    	buffer.append("                <p>You can use the OpenShift web console to enable additional capabilities via cartridges, add collaborator access authorizations, designate custom domain aliases, and manage domain memberships.</p>");
    	buffer.append("");
    	buffer.append("                <h3>Command Line Tools</h3>");
    	buffer.append("                <p>Installing the <a href=\"https://www.openshift.com/developers/rhc-client-tools-install\">OpenShift RHC client tools</a> allows you complete control of your cloud environment. Read more on how to manage your application from the command line in our <a href=\"https://www.openshift.com/user-guide\">User Guide</a>.");
    	buffer.append("                </p>");
    	buffer.append("");
    	buffer.append("                <h2>Development Resources</h2>");
    	buffer.append("                  <h3>JBoss Developer Studio</h3>");
    	buffer.append("                  <p>");
    	buffer.append("                    The JBoss Developer Studio is a full featured IDE with OpenShift integration built in.");
    	buffer.append("                    It gives you the ability to create, edit and deploy applications without having to");
    	buffer.append("                    leave the IDE.  Links to download, install and use the JBoss Developer Studio for Linux,");
    	buffer.append("                    Mac OS X, or Windows can be found on the");
    	buffer.append("                    <a href=\"https://www.openshift.com/developers/tools\">JBoss Developer Studio tools page.</a>");
    	buffer.append("                  </p>");
    	buffer.append("                  <h3>Debugging</h3>");
    	buffer.append("                  <p><a href=\"snoop.jsp\">View debugging information</a> about the server environment including memory pools.</p>");
    	buffer.append("                  <ul>");
    	buffer.append("                    <li><a href=\"https://www.openshift.com/developers\">Developer Center</a></li>");
    	buffer.append("                    <li><a href=\"https://www.openshift.com/user-guide\">User Guide</a></li>");
    	buffer.append("                    <li><a href=\"https://www.openshift.com/support\">OpenShift Support</a></li>");
    	buffer.append("                    <li><a href=\"http://stackoverflow.com/questions/tagged/openshift\">Stack Overflow questions for OpenShift</a></li>");
    	buffer.append("                    <li><a href=\"http://webchat.freenode.net/?randomnick=1&channels=openshift&uio=d4\">IRC channel at #openshift on freenode.net</a></li>");
    	buffer.append("                    <li><a href=\"http://git-scm.com/documentation\">Git documentation</a></li>");
    	buffer.append("                  </ul>");
    	buffer.append("");
    	buffer.append("          </section>");
    	buffer.append("        </div>");
    	buffer.append("");
    	buffer.append("        <footer>");
    	buffer.append("          <div class=\"logo\"><a href=\"https://www.openshift.com/\"></a></div>");
    	buffer.append("        </footer>");
    	buffer.append("</section>");
    	buffer.append("</body>");
    	buffer.append("</html>");
    	
        return buffer.toString();
    }

    public static void main(String[] args) throws Exception {
        SpringApplication.run(IndexController.class, args);
    }
}
